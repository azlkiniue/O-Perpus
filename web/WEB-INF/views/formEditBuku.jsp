     ...
     <div class="container">
      <div class="panel panel-primary">
        <div class="panel-heading">
          <div class="panel-title"><h1>FORM EDIT BUKU</h1></div>
        </div>
        <div class="panel-body">
            <a href="/Project_Library/buku" class='btn btn-default'><span class='glyphicon glyphicon-hand-left'></span> Kembali</a><br><br>
            <c:set var="model" value="${bukuModel}"/>
            <%
                BukuModel bukuModel = new BukuModel();
                bukuModel = (BukuModel) pageContext.getAttribute("model");
            %>
            <form name="add" method="post" action="">
           
                <input name="id" value="<%= bukuModel.getId() %>" hidden>
                <label for="judul">Judul Buku</label><br>
                <input name="judul" id="judul" placeholder="judul buku" class="form-control" value="<%= bukuModel.getJudul() %>" type="text"><br>
                
                <label for="tahun">Tahun Terbit</label><br>
                    <input name="tahun" id="tahun" placeholder="tahun buku" class="form-control" value="<%= bukuModel.getTahun()%>" type="text"><br>
                 
                <label for="penerbit">Penerbit Buku</label><br>
                    <input name="penerbit" id="penerbit" placeholder="penerbit buku" class="form-control" value="<%= bukuModel.getPenerbit() %>" type="text"><br>
                 
                <label for="penulis">Penulis Buku</label><br>
                    <input name="penulis" id="penulis" placeholder="penulis buku" class="form-control" value="<%= bukuModel.getPenulis()%>" type="text"><br>
                 
                <label for="harga">Harga Buku</label><br>
                    <input name="harga" id="harga" placeholder="harga buku" class="form-control" value="<%= bukuModel.getHarga()%>" type="text"><br>
                 
                 
                 <div class="btn-group">
                 <button class="btn btn-primary btn-fill" type="submit" value="Edit"><span class="glyphicon glyphicon-plus-sign"></span> Tambah</button>
                 <a href="/Project_Library/buku/delete?id=<%= bukuModel.getId() %>" class="btn btn-danger">Hapus <span class="glyphicon glyphicon-trash"></span></a>
                 </div>
