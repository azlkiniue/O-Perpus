    ...
    $(document).ready(function(){
      $('#check').click(function(){
        if ( $('#check').attr('class') == 'checkbox ct-red checked' ) {
            // do this
            document.getElementById("form").action="<% echo site_url("the_sociolib/postKembali/{$query[0]->no_transaksi}/{$telat}/false");%>"
        } else {
            // do that
            document.getElementById("form").action="<% $harga = ($query[0]->harga==0) ? 50000 : $query[0]->harga ; echo site_url("the_sociolib/postKembali/{$query[0]->no_transaksi}/{$telat}/{$harga}");%>"
        }
      });
    });
  </script>
